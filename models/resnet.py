...
class ResNet(object):
        ...
        for i in range(0, len(self.stages)):
            with tf.variable_scope("stages_%d_block_%d" % (i, 0)):
                x = self._bottleneck_residual(
                    x, self.filters[i], self.strides[i], "conv", self.is_training
                )
            for j in range(1, self.stages[i]):
                with tf.variable_scope("stages_%d_block_%d" % (i, j)):
                    x = self._bottleneck_residual(
                        x, self.filters[i], 1, "identity", self.is_training
                    )
    ...
    def _pre_padding_conv(
        self, name, x, kernel_size, out_channels, strides, bias=False
    ):
        """Convolution
